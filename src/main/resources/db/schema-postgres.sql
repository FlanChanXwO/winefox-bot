...
(
    ...
    CONSTRAINT uq_record_user_date UNIQUE (user_id, year, month, day)
);

CREATE TABLE IF NOT EXISTS deer_user_config
(
    user_id BIGINT NOT NULL PRIMARY KEY,
    allow_help      BOOLEAN DEFAULT TRUE, -- 是否允许被帮鹿
    last_retro_date DATE                  -- 上次使用补鹿功能的日期
);
